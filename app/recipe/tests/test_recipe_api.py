...
class PrivateRecipeApiTests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = create_user(email='user@example.com', password='test123', name='pedram')
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'description': 'sample description',
            'link': 'http://example.com',
            'tags': [{'name': 'Morgh'}, {'name': 'launch'}],
            'ingredients': [{'name': 'rob'}, {'name': 'zereshk'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
    ...
    def test_create_recipe_with_existing_ingredient(self):
        ...
        ingredient = Ingredient.objects.create(user=self.user, name='gerdo')
        payload = {
            'title': 'fesenjoon',
            'time_minutes': 60,
            'price': Decimal('6'),
            'description': 'sample description',
            'link': 'http://example.com',
            'tags': [{'name': 'meet'}, {'name': 'dinner'}],
            'ingredients': [{'name': 'gerdo'}, {'name': 'robe-anar'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        recipes = Recipe.objects.filter(user=self.user)
